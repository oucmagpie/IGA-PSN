    ...
    def forward(self, x):
        ...
        for i in range(len(img_split)):
            ...
            featt1 = self.attentionfusion(grad.view(gshape[0], gshape[1], gshape[2], gshape[3]),
                                          feat.view(shape[0], shape[1], shape[2], shape[
                                              3]))
            featt2 = self.attentionfusion(feat.view(shape[0], shape[1], shape[2], shape[3]),
                                          grad.view(gshape[0], gshape[1], gshape[2], gshape[
                                              3]))
            featss.append(featt2.view(-1))
            grads.append(featt1.view(-1))

            feats1.append(grad)
            feats2.append(feat)

        if self.fuse_type == 'mean':
            feat_fused = torch.stack(featss, 1).mean(1)
            grads_fused = torch.stack(grads, 1).mean(1)
            feat1_fused = torch.stack(feats2, 1).mean(1)
            grads1_fused = torch.stack(feats1, 1).mean(1)
        elif self.fuse_type == 'max':
            feat_fused, _ = torch.stack(featss, 1).max(1)
            grads_fused, _ = torch.stack(grads, 1).max(1)
            feat1_fused, _ = torch.stack(feats2, 1).max(1)
            grads1_fused, _ = torch.stack(feats1, 1).max(1)
